...
// #if defined(_MSC_VER) &&  (_MSC_VER >= 1400) /* VC8+ (VS2005) */
// 			::strncpy_s(m_pathOfProcess, MAX_PATH, pPathOfProcess, MAX_PATH - 1);
...
// #if defined(_MSC_VER) &&  (_MSC_VER >= 1400) /* VC8+ (VS2005) */
// 				::strncpy_s(szDllPath, _MAX_PATH, m_pathOfProcess, _MAX_PATH - 1);
// #else
...
// 		if (m_hMod == NULL) {
// #if __x86_64__
// 			const char* p = "native/win_x64/STLinkUSBDriver.dll";
// #else
// 			const char* p = "native/win_x86/STLinkUSBDriver.dll";
// #endif
// 			m_hMod = LoadLibraryA(p);
// 		}
...
STLinkIf_StatusT STLinkInterface::LoadStlinkLibrary2(QString &pPathOfProcess)
{
    STLinkIf_StatusT ifStatus = STLINKIF_NO_ERR;
    ...
    if (m_bApiDllLoaded == false) {
        // Load the STLinkUSBDriver library only once by instance
        if (pPathOfProcess.toStdString().c_str() != NULL) {
            // Memorize the path of process
#if defined(_MSC_VER) &&  (_MSC_VER >= 1400) /* VC8+ (VS2005) */
            ::strncpy_s(m_pathOfProcess, MAX_PATH, pPathOfProcess.toStdString().c_str(), MAX_PATH - 1);
            // Copy up to N-1 char to leave room for NULL; otherwise if the input is too long the invalid parameter handler is invoked.
            // We could also pass _TRUNCATE
#else
            ::strncpy(m_pathOfProcess, pPathOfProcess, MAX_PATH);
        ...
        if (m_hMod == NULL) {
            // First try from this DLL path
            // if (pPathOfProcess.toStdString().c_str() != NULL)
            if (!pPathOfProcess.isNull())
            {
                char szDllPath[_MAX_PATH];
                pPathOfProcess.append("STLinkUSBDriver.dll");
#if defined(_MSC_VER) &&  (_MSC_VER >= 1400) /* VC8+ (VS2005) */
                ::strncpy_s(szDllPath, _MAX_PATH, pPathOfProcess.toStdString().c_str(), _MAX_PATH - 1);
#else
                ::strncpy(szDllPath, m_pathOfProcess, _MAX_PATH);
#endif \
    // Note: Unicode is not supported (would require T_CHAR szDllPath, to include <tchar.h> \
